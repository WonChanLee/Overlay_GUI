...
def transmon_energy_single_flux(EC, EJ_sum, EJ_diff, phi, n = 3):
    '''Get the energy of a split junction transmon
    at flux phi
    
    up to the first n levels.
    
    Will inlcude the zeroth level for consistency with old fluxonium code
    
    This should probably eventually be replaced by a call to SC qubits,
    but AK is currently to lazy to figure out that install/syntax
    
    Currently it just uses the transmon fromula from Jens' paper and ignrores
    any breakdown of the transmon regime at low EJ/EC
    
    phi in unites of the flux quantum
    
    n = number of transmon levels computed
    '''
    ...
    # EJ_eff = EJ_sum *np.cos(np.pi*phi) * np.sqrt(1 + d**2 * (np.tan(np.pi*phi))**2 )
    EJ_eff = EJ_sum *np.abs(np.cos(np.pi*phi)) * np.sqrt(1 + d**2 * (np.tan(np.pi*phi))**2 )
    
    if EJ_eff <0:
        #stop trying to do idiotic math right at half flux
        qubit_Es = np.NaN*np.ones(n)
    else:
        linds = np.arange(0,n,1)
        qubit_Es = -EJ_eff + np.sqrt(8*EC*EJ_eff)*(linds + 0.5) - EC*(6*linds**2 +6*linds + 3)/12

    return qubit_Es

def transmon_energies(EC, EJ_sum, EJ_diff,
                      min_flux = -0.6, 
                      max_flux = 0.6, 
                      flux_points = 51,
                      n = 3):
    ...
    for find in range(0, len(flux)):
        phi = flux[find]
        energies[:,find] = transmon_energy_single_flux(EC=EC, EJ_sum=EJ_sum, EJ_diff=EJ_diff, phi=phi, n = n)
...
# check box
def transmon_plot_levels(ax, lines, energies, flux, cav_freq,
                         bare=True, 
                         twoPhoton=False, 
                         cavAssist=False,
                         freq_on_x_axis = True):
...
def overlay_GUI_transmon(fig, ax, lines, 
                         EC, EJ_sum, EJ_diff, 
                         cav_freq = 40, 
                         min_flux=-0.5, 
                         max_flux=0.5, 
    ...
    energies, flux = transmon_energies(EC, EJ_sum, EJ_diff, min_flux, max_flux, flux_points)
    lines = transmon_plot_levels(ax, lines, energies, flux, 
                                 cav_freq, 
                                 bare = bare,
                                 twoPhoton = twoPhoton,
                                 cavAssist = cavAssist,
